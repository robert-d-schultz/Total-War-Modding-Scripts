...
map_name = "wh3_main_combi_map_1"

xml_output_folder = "./terry_layer_output/"

binary_file_path = "C:\\Users\\rob\\Desktop\\terrain\\campaigns\\wh3_main_combi_map_1\\global_props.bin"

#It'll take like a minute for the script to run, there's lots of stuff in global_props.bin
#All the xml layer files will be in xml_output_folder
#The stuff you should put in your terry project file will be in terry_file_update.txt
...
with open(binary_file_path, 'rb') as binary_file:
    ...
    for _ in range(num_entities):
        ...
        #Set up the xml file
        print(entity_name.split(".")[1])
        if (region_to_layer[entity_name.split(".")[1]] == ""):
            id = "1" + ''.join(random.choice("01234567890abcdef") for _ in range(14))
            region_to_layer[entity_name.split(".")[1]] = id
    ...
    #Now go to each fastbin and get the props
    #for entity_name,entity_offset in entity_offsets.items():
    def some_function(entity_name, culture_mask):
        region = entity_name.split(".")[1]
        id = region_to_layer[region]
        xml_filepath = xml_output_folder + map_name + "." + id + ".layer"
        with open(xml_filepath, 'a') as xml_file:
            ...
            for _ in range(num_props):
                ...
                apply_height_patch2 = struct.unpack("<B", binary_file.read(1))[0] == 1
                apply_height_patch = apply_height_patch1 and apply_height_patch2
                ...
                if (is_decal):
                    xml_file.write(f"\n\t\t\t<ECDecal model_path=\"{path}\" parallax_scale=\"0\" tiling=\"0\" normal_mode=\"DNM_BLEND\""
                        + f"apply_to_terrain=\"{apply_to_terrain}\" apply_to_objects=\"{apply_to_objects}\" render_above_snow=\"{render_above_snow}\"/>")
                else:
                    xml_file.write("\n\t\t\t<ECPropMesh/>")
                ...
                xml_file.write(f"\n\t\t\t<ECPropHeightPatch apply_height_patch=\"{apply_height_patch}\" for_camera_height_map_only=\"false\"/>")
    ...
    #Call the abovefunction on the root node
    #Gotta do it this way because it's all a big tree and culture_masks are inherited from parents and shit
    
    some_function("terrain/campaigns/" + map_name + "/bmd_objects.bin", 0)


#Last step is to finish each xml file
#Also write a list
list_filepath = "./terry_file_update.txt"
with open(list_filepath, 'w') as list_file:
    list_file.write("\t<pc type=\"QTU::Scene\">")
    list_file.write("\n\t\t<data version=\"41\">")
...
for region in sorted(region_to_layer.keys(), key=get_third_part):
    id = region_to_layer[region]
    xml_filepath = xml_output_folder+ map_name + "." + id + ".layer"
    with open(xml_filepath, 'a') as xml_file:
        xml_file.write("\n\t</entities>\n\t<associations>\n\t\t<Logical/>\n\t\t<Transform/>\n\t</associations>\n</layer>")
    with open(list_filepath, 'a') as list_file:
        list_file.write("\n\t\t\t<entity id=\"" + id + "\" name=\"" + region + "\">")
